...
void GetMPUValues(){
  ...
  t = Wire.read();
  GyX = (float)(((t << 8) | Wire.read()) - offsetX) / 131; // 0x43 (GYRO_XOUT_H) & 0x44 (GYRO_XOUT_L)
  t = Wire.read();
  GyY = (float)(((t << 8) | Wire.read()) - offsetY) / 131; // 0x45 (GYRO_YOUT_H) & 0x46 (GYRO_YOUT_L)
  t = Wire.read();
  GyZ = (float)(((t << 8) | Wire.read()) - offsetZ  ) / 131; // 0x47 (GYRO_ZOUT_H) & 0x48 (GYRO_ZOUT_L)
  t = Wire.read();
}
...
void Debug(){
  ...
  if(showP){
    Serial.print("gPGX: "); Serial.print(gPGX); Serial.print(" // ");
    Serial.print("gPGY: "); Serial.print(gPGY); Serial.print(" // ");
...
void writePPM(Servo s, float value){
  float v = map(value, 0, 255, 1100, 2000);
